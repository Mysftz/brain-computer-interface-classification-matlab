...
disp('SBJ00 S00:  RUNNING  | Starting CNN Test - SBJ1-15, S1-3'); %Print in the terminal that indicates the start of the test
SBJS=fopen('/Users/mysftz/BCI/SBJ-S/SBJ1-15&S1-3.txt','r'); %Open a file outlining the all the subjects, sessions 1-3 and cell ranges for the matrix to print too
rt = tic; %Starting a timer, to measure the run time of each loop and overall test

for i=1:45 %15 subjects * sessions 1 to 3 = 45 sessions to run in total, also the file opening in line 3 contains 45 lines
    ...
    label=load(strcat('/Users/mysftz/BCI/Dataset/', sub, '/', sess, '/trainLabels.txt'));
    event=load(strcat('/Users/mysftz/BCI/Dataset/', sub, '/', sess, '/trainEvents.txt')); 
    runs=load(strcat('/Users/mysftz/BCI/Dataset/', sub, '/', sess, '/runs_per_block.txt')); 

    [sess_accuracy, predicted_object] = BCI_CNN_Function(event, label, runs, sess, sub, temp, testData, trainData); %Send all labels/ data to the function
    runtime=toc(rt); %saves loop run time and at the end overall run time

    writematrix(predicted_object, '/Users/mysftz/BCI/Results/CNN_Results_S1_3.xlsx', 'Range', matrixrange); %Save results data
    writematrix(sess_accuracy, '/Users/mysftz/BCI/Results/CNN_Accuracy_S1_3.xlsx', 'Range', cellrange); %Save accuracy data
    writematrix(runtime, '/Users/mysftz/BCI/Results/CNN_RunTime_S1_3.xlsx', 'Range', cellrange); %Save run time data
...
disp('SBJ00 S00:  ENDING   | Ending CNN Test - SBJ1-15, S1-3'); %Print in the terminal that indicates the end of the test
